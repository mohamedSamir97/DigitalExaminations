...
using ConfigurationManager = DigitalExaminations.Web.Extensions.ConfigurationManager;
...
namespace DigitalExaminations.Web.Controllers
{
    public class StudentsController : Controller
    {
        ...
        private string SaveStudentFile(IFormFile pictureFile)
        {
            ...
            var path = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot/file"); return SaveFile(path, pictureFile);
        }
        private string SaveFile(string path, IFormFile pictureFile)
        {
            ...
            var filename = Guid.NewGuid().ToString() + "." +
                pictureFile.FileName.Split('.')[pictureFile.FileName.Split('.').Length - 1];
